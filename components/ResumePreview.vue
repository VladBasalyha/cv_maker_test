<template>
	<div class="cvWrapper">
		<div ref="cv" tabindex="0" aria-label="CV preview" :class="['cv', 'bg-white', { blur: isLoading }, 'relative']">
			...
			<div class="cv__side">
				<h2 class="cv__name">{{ formSettings.name }} {{ formSettings.lastName }}</h2>
				...
				<section class="cv__section">
					...
					<div class="flex flex-col">
						<div v-if="formSettings.phoneNumber" class="cv__icon-wrapper">
							...
							<a :href="phoneNumberHref" rel="noopener">{{ formSettings.phoneNumber }}</a>
						</div>
				...
				<section class="cv__section">
					...
					<ul class="cv__bar">
						<li v-for="lang in formSettings.languages" :key="`preview${lang.lang}`" class="flex pr-4">
							<span>{{ lang.lang }}</span>
					...
					<div class="flex flex-col">
						<div v-if="formSettings.linkedin" class="cv__icon-wrapper">
							...
							<a target="_blank" rel="noopener" :href="`https://linkedin.com/in/${formSettings.linkedin}`">{{
								`linkedin.com/in/${formSettings.linkedin}`
							}}</a>
						</div>
						<div v-if="formSettings.twitter" class="cv__icon-wrapper">
							...
							<a target="_blank" rel="noopener" :href="`https://twitter.com/${formSettings.twitter}`">{{
								`twitter.com/${formSettings.twitter}`
							}}</a>
						</div>
						<div v-if="formSettings.github" class="cv__icon-wrapper">
							...
							<a target="_blank" rel="noopener" :href="`https://github.com/${formSettings.github}`">{{
								`github.com/${formSettings.github}`
							}}</a>
						</div>
						<div v-if="formSettings.website" class="cv__icon-wrapper">
							...
							<a target="_blank" rel="noopener" :href="formSettings.website">{{ formSettings.website }}</a>
						</div>
					</div>
				</section>
				<!-- // SOCIAL -->
...
<script lang="ts">
	import Vue from "vue";
	import { useContext, computed, ref, onMounted, onUnmounted } from "@nuxtjs/composition-api";
					...
					resizeObserver = new ResizeObserver(function (entries) {
						for (const entry of entries) {
							...
							if (newPages !== pages.value.length) {
								...
								const newHeightCm = Math.max(PAGE_HEIGHT_CM, newPages * PAGE_HEIGHT_CM);
								if (cv.value) {
...
<style lang="postcss" scoped>
	...
	.cv {
		...
		background-image: linear-gradient(to right, rgba(247, 250, 252, 1) 33%, rgba(255, 255, 255, 0) 0%);
