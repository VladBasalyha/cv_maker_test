				...
				<div class="flex flex-wrap gap-2 justify-start">
					<label
						...
						:class="[
							...
							'capitalize',
							{
								'form__btn--color-selected': color.color === formSettings.activeColor,
							},
						]"
...
<script lang="ts">
	import Vue from "vue";
	import { computed, onMounted, useContext, watch } from "@nuxtjs/composition-api";
	...
	export default Vue.extend({
		...
		setup() {
			...
			const { formSettings, uploadCV, clearForm, resetForm, setUpCvSettings } = useCvState();
			...
			watch(
				() => formSettings.value,
				(newValue, oldValue) => {
					localStorage.setItem(`cvSettings-${context.i18n.locale}`, JSON.stringify(newValue));
			...
			const formSettingsHref = computed(function getFormSettingsHref() {
				return `data:text/json;charset=utf-8,${encodeURIComponent(JSON.stringify({ formSettings: formSettings.value }))}`;
			});

			const availableLocales = computed(function getAvailableLocales() {
				return context.i18n.localeCodes.filter((locale: any) => !locale.includes("-"));
			});
